...
function CommentDetail({ postId, avatarUrl, username, ownerCaption, commentList }) {
  ...
  const handleDeleteComment = async (commentId) => {
    const newCommentListAfterDelete = commentWithUserInfo.filter(comment => comment.commentId !== commentId);
    try {
  ...
  useEffect(() => {
    ...
    else { //Tránh sideEffect này xảy ra trong lần đầu
    ...
    */
    try {
      const getCommentWithInfo = async () => {
        ...
        commentWithUserInfo ? setCommentWithUserInfo([...setCommentWithUserInfo, ...newData]) : setCommentWithUserInfo([...newData]);
      ...
      </div>
      {
        ...
        <div className="commentDetail__list">
          ...
          {countCommentDisplay < commentList.length ? (
            <button className="font-semibold text-[13px] text-gray-800" onClick={handleSeemoreComment}>
              Xem thêm bình luận
            </button>
          ) : (
            <></>
...
function CommentItem({ commentData, commentOfLoggedInUser, onDelete,...props }) {
          ...
          <div className="comment__name-and-content">
            ...
            <div className="comment__func-wrapper">
              ...
              <Dropdown
                ...
                content={<ul>
                  {commentOfLoggedInUser ? <li className="">
                    <button className="text-highlight-dropdown text-[14px] font-semibold w-full py-2 hover:bg-main-bg" onClick={() => onDelete(commentData.commentId)}>
                      Xóa bình luận
                    </button>
                  </li> : 
                  <>
                  ...
                  <li className="">
                      <button className=" text-[14px] font-semibold w-full py-2 hover:bg-main-bg" onClick={() => setToggleCommentDropdown(false)}>
                        Hủy
                      </button>
                    </li>
                </ul>}
