...
function DropdownEmoji({setValue}) {
    ...
    console.log(EMOJI.map(icon => icon.icons))
  return (
    <div className="emoji__wrapper">
        {
            EMOJI.map((emoji, index) => (
                <div className='emoji-box' key={index}>
                    <p className="mb-2 text-[13px] font-semibold text-gray-600">{emoji.label}</p>
                    <div className="grid md:grid-cols-7 grid-cols-5 gap-1">
                        {
                            emoji.icons.map((icon, index) => (
                                <button className="emoji-btn col-span-1" key={index} onClick={()=>handleOnclick(icon)}>
